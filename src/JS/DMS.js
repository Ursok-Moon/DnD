class DMScreen {
    ...
    try {
        ...
        // Verificar si hay cambios (nuevos personajes O modificaciones)
        ...
        if (hayCambios) {
            ...
            // Verificar si es un personaje nuevo o una modificación
            ...
            // SI HAY UN MODAL ABIERTO, FORZAR VERIFICACIÓN
    ...
    // ===== SISTEMA DE ACTUALIZACIÓN EN TIEMPO REAL DEL MODAL =====
   ...
   async verificarCambiosPersonaje() {
    ...
    try {
        // Buscar la versión actualizada del personaje en los datos más recientes
        ...
        if (versionActual !== this.ultimaVersionPersonaje) {
            ...
            this.modalAbierto = personajeActualizado; // Actualizar referencia
    ...
    // Aplicar colores personalizados si cambiaron
    ...
    // ===== NUEVAS SECCIONES (MANTENIENDO EL MISMO ESTILO) =====
...
if (personaje.deathSaves) {
    ...
    successes.forEach((checked) => {
        html += `<span style="width: 40px; height: 40px; border: 3px solid var(--accent-gold); border-radius: 8px; display: flex; align-items: center; justify-content: center; background: ${checked ? '#4CAF50' : 'rgba(255,255,255,0.5)'};"></span>`;
    });
    html += `</div></div>`;
    ...
    html += `<div style="display: flex; gap: 10px; justify-content: center;">`;
    fails.forEach((checked) => {
        html += `<span style="width: 40px; height: 40px; border: 3px solid var(--accent-gold); border-radius: 8px; display: flex; align-items: center; justify-content: center; background: ${checked ? '#ff4444' : 'rgba(255,255,255,0.5)'};"></span>`;
    });
    html += `</div></div>`;
    html += `</div>`;
    ...
    aplicarColoresPersonaje(modal, colores) {
        ...
        // Aplicar colores de texto si existen
    ...
    // Método auxiliar para ajustar color (oscurecer/clarecer)
    ajustarColor(hex, percent) {
        // Si no es hex, devolver el original
    ...
    // ===== CONFIGURAR EDITOR DE NOTAS ENRIQUECIDO =====
    setupRichTextEditor() {
